-- ============================================================
-- BUG-161: complete_ride_and_pay validated that p_driver_id matches
-- v_ride.driver_id but did NOT validate that the caller is that
-- driver. The function is SECURITY DEFINER, granted to authenticated,
-- so any logged-in user who knows a ride's driver_id (visible to the
-- ride's customer via get_assigned_driver_info from BUG-123) could
-- call:
--
--   supabase.rpc('complete_ride_and_pay', {
--     p_ride_id: my_in_progress_ride_id,
--     p_driver_id: actual_driver_id,
--     p_actual_distance_m: 0,
--     p_actual_duration_s: 0,
--   });
--
-- Effect: a customer could complete their own in-progress ride
-- early with bogus metrics, paying only base_fare + min_fare instead
-- of the real distance/duration-based fare. Driver loses earnings,
-- platform commission shrinks. A bystander could also complete a
-- ride they're not part of, denial-of-servicing real users.
--
-- Fix: gate the function. The caller (auth.uid()) must equal the
-- driver's user_id, or be an admin/super_admin. The legitimate
-- caller is the driver app's "complete ride" handoff after the
-- driver-side phone has measured GPS distance/duration.
-- ============================================================
...
BEGIN
  ...
  -- BUG-161: caller must be the driver themselves or admin.
  ...
  -- (rest of function body unchanged from prior version — payment dispatch
  -- by payment_method: tricicoin, mixed, tropipay, corporate, cash)
...
COMMENT ON FUNCTION public.complete_ride_and_pay(uuid, uuid, integer, integer) IS
  'BUG-161: caller must be the assigned driver (auth.uid() = driver_profiles.user_id) or admin. Customers and bystanders cannot complete a ride to undercharge themselves with bogus distance/duration metrics.';
