-- ============================================================
-- Migration 00031: Fare Splitting
-- Allows riders to split ride costs among multiple passengers.
-- Only supported for 'tricicoin' payment method.
-- ============================================================

-- 1. Add is_split flag to rides
ALTER TABLE rides ADD COLUMN IF NOT EXISTS is_split BOOLEAN NOT NULL DEFAULT false;

-- 2. Create ride_splits table
...
CREATE INDEX idx_ride_splits_ride ON ride_splits(ride_id);
CREATE INDEX idx_ride_splits_user ON ride_splits(user_id);
CREATE INDEX idx_ride_splits_pending ON ride_splits(user_id, payment_status)
  WHERE payment_status = 'pending';

-- 3. RLS
ALTER TABLE ride_splits ENABLE ROW LEVEL SECURITY;

-- Owner (ride requester) and invited user can read splits
CREATE POLICY "split_select" ON ride_splits FOR SELECT USING (
  user_id = auth.uid()
  OR ride_id IN (SELECT id FROM rides WHERE customer_id = auth.uid())
  OR is_admin()
);

-- Only ride owner can insert splits (before in_progress)
CREATE POLICY "split_insert" ON ride_splits FOR INSERT WITH CHECK (
  ride_id IN (
    SELECT id FROM rides
    WHERE customer_id = auth.uid()
      AND status IN ('searching', 'accepted', 'driver_en_route', 'arrived_at_pickup')
  )
);

-- Only ride owner can delete splits (before in_progress)
CREATE POLICY "split_delete" ON ride_splits FOR DELETE USING (
  ride_id IN (
    SELECT id FROM rides
    WHERE customer_id = auth.uid()
      AND status IN ('searching', 'accepted', 'driver_en_route', 'arrived_at_pickup')
...
-- Invited user can update their own split (to accept)
CREATE POLICY "split_update" ON ride_splits FOR UPDATE USING (
  user_id = auth.uid()
) WITH CHECK (
  user_id = auth.uid()
);
...
-- 4. Enable realtime
ALTER PUBLICATION supabase_realtime ADD TABLE ride_splits;

-- ============================================================
-- 5. Update complete_ride_and_pay() to handle split payments
-- ============================================================
...
BEGIN
  ...
  ELSIF v_ride.payment_method = 'tropipay' THEN
    -- === TROPIPAY PAYMENT (no change) ===
    NULL;

  ELSE
    -- === CASH / MIXED PAYMENT (no change) ===
    v_driver_account_id := ensure_wallet_account(v_driver_user_id, 'driver_cash');
    v_platform_account_id := ensure_wallet_account(v_platform_user_id, 'platform_revenue');
