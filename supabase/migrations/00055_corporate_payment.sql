-- ============================================================
-- Migration 00055: Corporate Payment in complete_ride_and_pay
-- ============================================================
-- Adds the 'corporate' payment method branch to the ride
-- completion function. Corporate rides debit the corporate
-- account's wallet, record in corporate_rides, and update
-- monthly spending.
-- ============================================================
...
BEGIN
  ...
  -- 2b. Calculate surge multiplier (uses helper with correct JOIN)
  ...
  ELSIF v_ride.payment_method = 'corporate' THEN
    -- === CORPORATE PAYMENT ===
    -- Debit from the corporate account's wallet (corporate_cash),
    -- credit driver and platform, record in corporate_rides,
    -- and update monthly spend.

    -- 1. Find the corporate account owner to get their corporate_cash wallet
    ...
    -- 2. Create ledger transaction: debit corporate wallet, credit driver + platform
    INSERT INTO ledger_transactions (
      id, idempotency_key, type, status, reference_type, reference_id, description, created_by
    ) VALUES (
      gen_random_uuid(),
    ...
    -- 3. Record in corporate_rides and update monthly spend (atomic)
    ...
    -- 4. Insurance premium (debit from corporate wallet)
    IF v_insurance_premium_trc > 0 THEN
      SELECT balance INTO v_customer_balance FROM wallet_accounts WHERE id = v_customer_account_id;
      SELECT balance INTO v_platform_balance FROM wallet_accounts WHERE id = v_platform_account_id;
