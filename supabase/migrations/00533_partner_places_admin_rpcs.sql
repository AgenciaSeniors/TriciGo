...
--
-- Why these are SECURITY DEFINER rather than plain table access: 00529 dropped
...
-- coupons at a business and the rate-limit bucket key (00531). The admin needs
...
COMMENT ON FUNCTION public.admin_list_partner_places() IS
  '00533 Admin list. SECURITY DEFINER so it can read partner_places.validation_token, which 00529 revoked from authenticated at the column level. Gated on is_admin().';
COMMENT ON FUNCTION public.admin_upsert_partner_place(UUID, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INT, INT, INT, BOOLEAN, TIMESTAMPTZ) IS
  '00533 Admin insert/update. Never touches validation_token: businesses bookmark tricigo.com/v/<token> and rotating it on an edit would break redemption.';
