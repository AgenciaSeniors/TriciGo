-- BUG-254: update_ride_status_v2 was raising
--   "column \"status\" is of type ride_status but expression is of type text"
-- when the driver tapped "En camino al pasajero" / advanceStatus paths.
-- The parameter p_new_status is declared TEXT and the UPDATEs did not
-- cast back to the enum. Fix: cast p_new_status::ride_status in every
-- UPDATE that sets the status column.
...
BEGIN
  ...
  ELSE
    -- BUG-254: cast text -> ride_status to match column type
    UPDATE rides SET status = p_new_status::ride_status, updated_at = now() WHERE id = p_ride_id;
    IF p_new_status = 'driver_en_route' THEN
      UPDATE rides SET driver_en_route_at = COALESCE(driver_en_route_at, now()) WHERE id = p_ride_id;
