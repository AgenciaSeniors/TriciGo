...
CREATE TABLE IF NOT EXISTS public.partner_places (
  ...
  -- reasoning, and 00534 for how it is used.
...
-- that applied an earlier 00532) to the same shape.
--
...
-- 00533: that alphabet drops 0/1/I/L/O because a coupon code is read aloud
...
-- trigger in 00533: that predicate is
...
-- capabilities 00534 exists to deny: redeeming coupons at a shop they are not
...
-- (see 00533): the statement succeeds and changes nothing.
--
...
-- not apply to it, which is exactly how 00534's validate/redeem reach it. Do
...
VALUES
  ...
  -- window (00534 keys the bucket on partner_place_id). Configurable rather
...
COMMENT ON COLUMN public.partner_places.validation_token IS
  '00532 SECRET. 12 hex chars identifying this business on tricigo.com/v/<token>. It is both the authorisation to validate coupons and the rate-limit bucket key (00534). Revoked from anon/authenticated at the column level — never expose it through a table SELECT.';
