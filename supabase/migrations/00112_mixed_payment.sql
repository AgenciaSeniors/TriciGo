...
-- Add wallet_ratio, wallet_amount_cup, cash_amount_cup to rides
-- Update complete_ride_and_pay to handle 'mixed' payment method
-- ============================================================

-- 1. Add mixed payment columns to rides
ALTER TABLE rides
  ADD COLUMN IF NOT EXISTS wallet_ratio NUMERIC(3,2) DEFAULT 0,
  ADD COLUMN IF NOT EXISTS wallet_amount_cup INTEGER DEFAULT 0,
  ADD COLUMN IF NOT EXISTS cash_amount_cup INTEGER DEFAULT 0;

-- 2. Recreate complete_ride_and_pay with mixed payment branch
...
BEGIN
  ...
  ELSIF v_payment_method_text = 'mixed' THEN
    -- === MIXED PAYMENT: part wallet, part cash ===
    -- Calculate wallet vs cash split based on wallet_ratio
    ...
    -- Cap wallet_amount to available balance (safety check)
    v_wallet_amount := LEAST(GREATEST(v_wallet_amount, 0), v_customer_balance);
    v_cash_amount := v_final_fare_trc - v_wallet_amount;

    -- Update ride with actual wallet/cash amounts
    UPDATE rides SET
      wallet_amount_cup = v_wallet_amount,
      cash_amount_cup = v_cash_amount
    WHERE id = p_ride_id;

    -- Wallet debit from customer (if wallet portion > 0)
    ...
    -- Driver earnings credit (commission on FULL fare, not just wallet part)
    INSERT INTO ledger_transactions (id, idempotency_key, type, status, reference_type, reference_id, description, created_by)
    ...
    -- Platform commission credit (commission on FULL fare)
    INSERT INTO ledger_entries (transaction_id, account_id, amount, balance_after)
    VALUES (v_txn_id, v_platform_account_id, v_commission_amount, v_platform_balance + v_commission_amount);

    -- Update wallet balances for driver and platform
    UPDATE wallet_accounts SET balance = balance + v_driver_earnings WHERE id = v_driver_account_id;
    UPDATE wallet_accounts SET balance = balance + v_commission_amount WHERE id = v_platform_account_id;

    -- Insurance premium handling for mixed payments
    IF v_insurance_premium_trc > 0 THEN
      SELECT balance INTO v_customer_balance FROM wallet_accounts WHERE id = v_customer_account_id;
      SELECT balance INTO v_platform_balance FROM wallet_accounts WHERE id = v_platform_account_id;
