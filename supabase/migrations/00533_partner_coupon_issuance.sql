...
BEGIN
  ...
  -- serve it. Fine here — the table is small and admin-curated. See 00532.
  FOR v_place IN
    SELECT pp.*
...
COMMENT ON FUNCTION public._generate_coupon_code() IS
  '00533 Six-char coupon code over a 31-symbol alphabet (no 0/1/I/L/O), drawn from pgcrypto with rejection sampling so the distribution stays uniform.';
COMMENT ON FUNCTION public.tg_rides_issue_partner_coupons() IS
  '00533 Issues one partner coupon per matching place when a PASSENGER ride completes inside its radius (cargo excluded in the trigger WHEN clause). Never blocks the ride.';
