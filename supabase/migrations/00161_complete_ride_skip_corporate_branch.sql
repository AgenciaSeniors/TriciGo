-- ============================================================
-- BUG-101: complete_ride_and_pay was double-counting corporate
-- rides. The function's final ELSE branch ran `cash_commission`
-- for ANY payment method not explicitly handled (tricicoin,
-- mixed, tropipay). That ELSE caught both 'cash' AND 'corporate'.
--
-- For corporate rides, the AFTER UPDATE trigger
-- handle_corporate_ride_completion (migration 00151) already:
--   - debits corporate_cash by the full fare
--   - credits driver_cash by driver_earnings
--   - credits platform_revenue by commission_amount
--
-- But complete_ride_and_pay's cash_commission block ALSO ran for
-- corporate rides, adding:
--   - driver_cash: -commission  (WRONG — driver didn't collect cash)
--   - platform_revenue: +commission (WRONG — double counted)
--
-- Fix: add an explicit 'corporate' -> NO-OP branch so only the
-- trigger handles the side effects. 'cash' still runs the
-- cash_commission flow via the ELSE fallthrough.
--
-- Payment methods in use (per operator clarification):
--   cash, tricicoin, mixed, corporate
-- Stripe: recharge-only, never reaches this function for rides.
-- Tropipay: deprecated (branch kept as no-op for historical rides).
-- ============================================================
...
BEGIN
  ...
  ELSIF v_payment_method_text = 'tropipay' THEN
    -- Deprecated. Branch kept for legacy rides; operators have moved to
    -- stripe for wallet recharge + tricicoin/mixed/cash/corporate for rides.
    NULL;

  ELSIF v_payment_method_text = 'corporate' THEN
    -- BUG-101 fix: corporate rides are fully handled by the
    -- handle_corporate_ride_completion AFTER UPDATE trigger from
    -- migration 00151. Running the cash_commission branch here
    -- (as we did before 00161) double-debited the driver and
    -- double-counted the platform commission.
    NULL;

  ELSE
    -- 'cash' only — driver collects from customer, owes commission to
    -- platform from driver_cash wallet.
    v_driver_account_id := ensure_wallet_account(v_driver_user_id, 'driver_cash');
    v_platform_account_id := ensure_wallet_account(v_platform_user_id, 'platform_revenue');
    SELECT balance INTO v_driver_balance FROM wallet_accounts WHERE id = v_driver_account_id;
    SELECT balance INTO v_platform_balance FROM wallet_accounts WHERE id = v_platform_account_id;
