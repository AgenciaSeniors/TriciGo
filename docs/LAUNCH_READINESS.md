...
| **Offline / network resilience** | ⚠️ **CONDITIONAL** — good GPS buffering + banners, but ride-state changes aren't queued offline (G2), no error boundaries (G3), Sentry not capturing (G4). UX/observability gaps, not data-integrity gaps. |
| **Battle-testing at scale** | ⚠️ Only 7 approved drivers / 51 completed rides. Logic is correct; throughput is unproven. |

**Bottom line:** the **money-and-state core is launch-safe** — you cannot double-book a driver or a ride, and the dispatcher degrades gracefully. The conditional items (G1–G4) are about *graceful degradation and visibility under bad networks*, plus one commission-debt regression. None corrupt data; all are worth closing before a public launch with real driver volume.
...
**The gaps (see G2–G4 below):**
- **Ride-state changes** (accept / complete / cancel) are **not** queued offline → if the network drops at that instant, the action fails and is lost silently; the counterpart sees stale state.
- **No React error boundaries** → an unhandled render error can white-screen the whole app.
- **Sentry initialized but not capturing** → these failures aren't reported, so you'd be blind to them in production.

**Severity:** medium (UX + observability; not data corruption). **Verdict:** ⚠️ CONDITIONAL.
...
### G2 — Offline queue not wired for ride-state changes ⚠️ (UX)
`offlineQueue` (`packages/utils/src/offlineQueue.ts`) + `executeOrQueue` (`packages/api/src/lib/offlineQueue.ts`) exist and already register `ride.cancel`/`review.submit`/`incident.sos`, but the driver never routes accept/complete through them, and `useOfflineSync.ts` only flushes reviews.

**Fix:** route driver accept/complete/cancel through `executeOrQueue`; register the missing actions in `offlineMutations.ts`; flush on offline→online in both apps. RPCs are already idempotent (status guards), so replay is safe.

### G3 — No React error boundaries ⚠️ (stability)
**Fix:** add an `<ErrorBoundary>` in `packages/ui`, wrap the tree in `apps/{client,driver}/app/_layout.tsx`, with a "something went wrong / retry" fallback.

### G4 — Sentry not capturing exceptions ⚠️ (observability)
Initialized in `apps/{client,driver}/src/lib/sentry.ts` but no `captureException`.
**Fix:** capture from the ErrorBoundary and from critical ride-flow `.catch` blocks; user-facing text via `getErrorMessage` (`packages/utils/src/errors.ts`).
...
- [ ] **G2** — queue ride-state changes offline + flush on reconnect.
- [ ] **G3** — error boundaries in both apps.
- [ ] **G4** — Sentry `captureException` wired.
