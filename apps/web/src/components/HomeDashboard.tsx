...
import type { Ride, PartnerPlace } from '@tricigo/types';
...
export function HomeDashboard({ userId, onRebook, onApplyPromo }: {
  ...
  useEffect(() => {
    ...
    // Nearby partner places. The reader swallows a missing RPC and returns [],
    // so this is safe before the migrations are applied to prod.
    //
    // Geolocation only — on denial, error or timeout we render nothing, the
    // same rule the mobile carousel follows. Showing a bakery that might be in
    // another province is worse than showing none.
    //
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginTop: '1.75rem' }}>
      ...
      {places.length > 0 && (
        <section>
          <p style={sectionLabel}>{t('home.dashboard_partner_places', { defaultValue: 'Lugares con descuento' })}</p>
