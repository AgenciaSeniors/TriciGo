...
export default function BlogPage() {
  ...
  return (
    <main style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>{t('blog.subtitle')}</p>
      {loading ? (
        <p style={{ color: 'var(--text-secondary)' }}>{t('blog.loading')}</p>
      ) : posts.length === 0 ? (
        <p style={{ color: 'var(--text-secondary)' }}>{t('blog.no_posts')}</p>
      ) : (
              ...
              <Link key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <article style={{ border: '1px solid var(--border-light)', borderRadius: '1rem', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    ...
                    {post.published_at && (
                      <time dateTime={post.published_at} style={{ color: 'var(--text-tertiary)', fontSize: '0.8rem', display: 'block' }}>
