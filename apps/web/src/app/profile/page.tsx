...
function IconDevices() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2" /><line x1="12" y1="18" x2="12.01" y2="18" /></svg>; }
...
const iconMap: Record<string, () => React.JSX.Element> = {
  ...
  'profile.menu_devices': IconDevices,
...
interface MenuItem {
  labelKey: string;
  href: string;
  /** Fallback label when the key isn't in the locale bundle yet. */
  defaultValue?: string;
}
...
const menuGroups: { titleKey: string; items: MenuItem[] }[] = [
  {
    ...
    items: [
      ...
      { labelKey: 'profile.menu_ride_preferences', href: '/profile/ride-preferences', defaultValue: 'Preferencias de viaje' },
      { labelKey: 'profile.menu_recurring_rides', href: '/profile/recurring-rides' },
      { labelKey: 'profile.menu_devices', href: '/profile/devices', defaultValue: 'Tus dispositivos' },
    ],
  },
  {
          ...
          <div key={group.titleKey} className="profile-menu-group">
            ...
            <div className="profile-menu-group-items">
              {group.items.map((item) => {
                const label = t(item.labelKey, { defaultValue: item.defaultValue });
