 ...
 * after an external change (wallet credit, earnings, rating, approval status,
 * penalty…) until a full restart. The wallet screen already does this with
 * `useFocusEffect`; this hook is the canonical primitive so every mutable-data
 * screen behaves the same. See CLAUDE.md → "Auditoría de frescura de datos
 * (stale-on-mount)".
 *
