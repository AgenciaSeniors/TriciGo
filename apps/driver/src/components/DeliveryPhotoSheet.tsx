 ...
 * Shown before a delivery ride can be completed.
 */
export function DeliveryPhotoSheet({ rideId, onPhotoUploaded, onSkip }: DeliveryPhotoSheetProps) {
  ...
  const uploadAndConfirm = useCallback(async () => {
    ...
    setUploading(true);
    try {
      await deliveryService.uploadDeliveryPhoto(rideId, photoUri);
      triggerHaptic('success');
      Toast.show({
        type: 'success',
        text1: t('trip.delivery_photo_saved', { defaultValue: 'Foto de entrega guardada' }),
      });
      onPhotoUploaded();
    } catch (err) {
  ...
  return (
    <Card variant="filled" padding="lg" className="bg-neutral-800 mb-4">
      ...
      <Text variant="bodySmall" color="secondary" className="text-center mb-4">
        {t('trip.delivery_photo_desc', {
          defaultValue: 'Tome una foto del paquete entregado como comprobante',
        })}
      </Text>
