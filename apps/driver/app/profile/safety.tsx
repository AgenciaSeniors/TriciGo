    ...
    <Screen scroll bg="dark" statusBarStyle="light-content" padded>
      <View className="pt-4">
        <View className="flex-row items-center mb-6">
          ...
          <Text variant="h3" color="inverse">{t('safety.title')}</Text>
        </View>

        <Text variant="bodySmall" color="inverse" className="mb-4 opacity-60">
          {t('safety.desc')}
        </Text>
            ...
            <View className="mt-3 pt-3 border-t border-neutral-700">
              {DRIVER_TIPS.map((tipKey, idx) => (
                <View key={tipKey} className="flex-row items-start mb-2">
                  <Text variant="caption" color="inverse" className="mr-2 opacity-50">
                    {idx + 1}.
                  </Text>
                  <Text variant="bodySmall" color="inverse" className="flex-1">
                    {t(`safety.${tipKey}`)}
                  </Text>
                </View>
              ))}
        ...
        <Card variant="filled" padding="md" className="mb-6 bg-neutral-800">
          ...
          {incidents.length === 0 ? (
            <Text variant="caption" color="inverse" className="text-center py-2 opacity-50">
              {t('safety.no_reports')}
            </Text>
          ) : (
            incidents.slice(0, 5).map((incident) => (
              <View key={incident.id} className="flex-row items-center justify-between py-2 border-t border-neutral-700">
                <View className="flex-1">
                  <Text variant="bodySmall" color="inverse">{getReportTypeLabel(incident.type)}</Text>
                  <Text variant="caption" color="inverse" className="opacity-50">
