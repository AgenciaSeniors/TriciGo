# Content Rating — TriciGo (Pasajero)

> Answers ready to paste/select in **Google Play Console → Policy → App content → Content ratings → Start questionnaire (IARC)** and **App Store Connect → App → Age Rating → Edit**.
>
> TriciGo Pasajero is a ride-hailing/urban-mobility utility app. It contains **no objectionable content**. The only "interactive elements" are: (a) rider↔driver **in-app chat during an active trip**, and (b) **location sharing** between rider and assigned driver during a trip. Net result: **Everyone / 4+**, with the standard "Users Interact" + "Shares Location" descriptors.

---

## 1. Google Play — IARC questionnaire

**Category:** App (not a game) → "Utility, Productivity, Communication, or Other".

**Email for IARC certificate:** soporte@tricigo.com

| Question | Answer |
|---|---|
| Does the app contain violence (cartoon, fantasy, or realistic)? | **No** |
| Does the app contain sexual or suggestive content / nudity? | **No** |
| Does the app contain profanity or crude humor? | **No** |
| Does the app reference or depict controlled substances (drugs, alcohol, tobacco)? | **No** |
| Does the app contain gambling (simulated or real-money)? | **No** |
| Does the app contain content that could be frightening/disturbing to young users? | **No** |
| Is the app's content user-generated, and can users interact or exchange content/messages? | **Yes** — riders and the assigned driver can exchange short text messages **during an active trip only** (in-app chat). No public feed, no profiles browsing, no media sharing. |
| Does the app share the user's current physical location with other users? | **Yes** — during an active trip the rider sees the assigned driver's live location and the driver sees the pickup point. Not shared outside the trip. |
| Does the app allow users to purchase digital goods? | **No digital goods.** Users may top up **trip credit** used to pay for **real-world transportation** (processed by NETOPIA, a third-party payment processor). This is a real-world service, not digital content. |
| Does the app contain ads? | **No** |
| Is the app a "news" app or does it primarily deliver content? | **No** — it is a transportation utility. |

**Expected rating:** ESRB **Everyone** · PEGI **3** · USK **0** · etc. · with **"Users Interact"** and **"Shares Location"** interactive-elements labels.

---

## 2. Apple App Store — Age Rating questionnaire

Set every content type to its lowest frequency:

| Content description | Selection |
|---|---|
| Cartoon or Fantasy Violence | **None** |
| Realistic Violence | **None** |
| Prolonged Graphic or Sadistic Realistic Violence | **None** |
| Profanity or Crude Humor | **None** |
| Mature/Suggestive Themes | **None** |
| Horror/Fear Themes | **None** |
| Medical/Treatment Information | **None** |
| Alcohol, Tobacco, or Drug Use or References | **None** |
| Simulated Gambling | **None** |
| Sexual Content or Nudity | **None** |
| Graphic Sexual Content and Nudity | **None** |
| Contests | **None** |
| Unrestricted Web Access | **No** *(NETOPIA opens a single fixed hosted payment page in an in-app browser session — not a general/unrestricted web browser)* |
| Gambling | **No** |
| Made for Kids | **No** |

**Expected rating: 4+.**

> Note: rider↔driver in-app messaging and live location sharing are disclosed in **App Privacy** (PrivacyInfo.xcprivacy / data-safety) and in the review notes; they do not raise the Apple age rating above 4+.
