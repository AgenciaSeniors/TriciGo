...
describe('mergeProguardRules', () => {
  it('keeps everything prebuild generated', () => {
    const out = mergeProguardRules(GENERATED, '-keep class app.tricigo.client.** { *; }');
  ...
  it('appends the TriciGo rules under a sentinel', () => {
    ...
    expect(out).toContain('-keep class app.tricigo.client.** { *; }');
  });

  // A second prebuild over the same tree must not stack the rules again.
  it('is idempotent', () => {
    const once = mergeProguardRules(GENERATED, '-keep class app.tricigo.client.** { *; }');
    const twice = mergeProguardRules(once, '-keep class app.tricigo.client.** { *; }');
    expect(twice).toBe(once);
    expect(twice.split(SENTINEL).length - 1).toBe(1);
  });

  it('works from an empty file', () => {
    const out = mergeProguardRules('', '-keep class app.tricigo.client.** { *; }');
    expect(out).toContain('-keep class app.tricigo.client.** { *; }');
  });
});
