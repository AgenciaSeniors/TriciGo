 ...
 * spend-only (rides). Recipient lookup by shareable code (QR or typed)
 * or by phone. Mirrors the wallet/referral screen style.
 */
export default function GiftScreen() {
  ...
  // Deep link / QR: tricigo://gift/<code> opens this screen with ?code=,
  // which auto-resolves the recipient.
  useEffect(() => {
    if (codeParam) {
      setMode('code');
      setQuery(codeParam);
            ...
            <Card variant="outlined" padding="md" className="mb-4">
              ...
              {Platform.OS !== 'web' ? (
                <Pressable onPress={() => setScannerOpen(true)} className="flex-row items-center justify-center mt-3" hitSlop={8}>
                  ...
                  <Text variant="bodySmall" color="primary" className="ml-2 font-medium">
          ...
          <Card variant="filled" padding="lg" className="mb-6 mt-2 bg-primary-50 items-center">
            ...
            {QRCode && myCode ? (
              <View className="mb-3 p-3 bg-white rounded-2xl">
                <QRCode value={`tricigo://gift/${myCode}`} size={160} />
              </View>
            ) : null}
            <Button
